...
router.post('/fat/apprenticeship-or-provider', (req, res) => {
	var searchProvider = req.body['search-provider']
	var searchLocation = req.body['search-location']
	var searchApprenticeship = req.body['search-apprenticeship']

	if(searchProvider.length > 0 && searchLocation.length == 0 && searchApprenticeship.length == 0){
...
router.get('/fat/provider', (req, res) => {
	var ukprn = req.query['ukprn'] || '10003753'
	var response = []
